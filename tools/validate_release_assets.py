"""Static release-asset validation for the SmolVLM-500M-Instruct captioning/VQA DIMER pipeline.
...
CODE_MARKERS = (
    'from smolvlm_vision_language_pipeline import DECODING, DEFAULT_MAX_NEW_TOKENS, MAX_IMAGE_SIDE, M'
    'AX_IMAGES, MAX_NEW_TOKENS, MAX_TEXT_CHARS',
    'from smolvlm_vision_language_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, SmolVLMPipelin'
    'e, stage_missing_files, verify_snapshot',
    ...
    "ceilings = {'MAX_IMAGES': MAX_IMAGES, 'MAX_IMAGE_SIDE': MAX_IMAGE_SIDE, 'MAX_TEXT_CHARS': MAX_TE"
    "XT_CHARS, 'MAX_NEW_TOKENS': MAX_NEW_TOKENS, 'DEFAULT_MAX_NEW_TOKENS': DEFAULT_MAX_NEW_TOKENS, 'D"
    "ECODING': DECODING}",
    "raise ValueError('input rejected before model execution: '",
    "'greedy_as_requested': all(a['generation']['do_sample'] is False and a['generation']['decoding']"
    ' == DECODING for a in answers)',
...
MARKDOWN_MARKERS = (
    '**Capability:** image + text to text generation (captioning and visual question answering)',
    '**greedy by default**',
    '**free text with no score, no probability and no correctness signal**',
    'claimed its corners touched the image edges',
    'SmolLM2-360M-Instruct text decoder, which is itself already a DIMER language-model profile',
